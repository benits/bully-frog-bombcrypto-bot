...
class Application:
    ...
    def start(self):
        ...
        self.log.console('Starting bot...', services=True,
                         emoji='🤖', color='green')
        self.log.console('"Commands: \n\n /id \n /print \n /map \n /bcoin \n /workall \n /restall \n /refresh \n\n /stop - Stop the bot \n /donation - Donate to help the project"', services=True,
                         emoji='🤖', color='green')
...
""".format(localVersion[0], localVersion[1], gitHubVersion[0], gitHubVersion[1])
    ...
    def gitHubVersion(self):
        self.importLibs()
        data = requests.get(
            'https://raw.githubusercontent.com/benits/bully-frog-bombcrypto-bot/main/config/version.yaml')
        try:
        ...
        if versionLocalApp >= versionGithubApp:
            ...
            if len(diff) > 0 and 'dictionary_item_added' in diff:
                ...
                try:
                    ...
                    print(
                        Fore.WHITE + '\nSee: https://github.com/benits/bully-frog-bombcrypto-bot/blob/main/config/EXAMPLE-config.yaml' + Fore.RESET)
                    print(
                        Fore.GREEN + '\n*********************************************************************************' + Fore.RESET)
